...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  case 1:
    ...
    {
      PowerLock = true;
      client.publish("PowerLock", "ON");
    }
    else if (valuejson == 1)
    {
      PowerLock = false;
      client.publish("PowerLock", "OFF");
    }
    break;
  ...
  case 5:
    if (valuejson == 5 && passjson == passon && !PowerLock)
    {
      ...
      client.publish("PowerON", "ON");
    }
    else if (valuejson == 9 && passjson == passoff && !PowerLock)
    {
      client.publish("PowerON", "OFF");
    }
    break;
